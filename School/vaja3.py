...
def text_to_image(image_path, text, position, font_size=24, font_color=(255, 255, 255)):
    ...
    font = ImageFont.truetype(font="/usr/share/fonts/truetype/ubuntu/Ubuntu[wdth,wght].ttf", size=font_size)
...
def merge_images(folder_path, rows=1, cols=1, userScale=False):
    """
    ...
    img_width = int(round(width / num_of_images,0))
    img_height = int(round(height / num_of_images,0))



    real_width = int(img_width * (round(num_of_images/2, 0)))
    real_height = int(img_height * (round(num_of_images/2, 0)))
